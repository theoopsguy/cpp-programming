...
{
    if (row >= col)
        arr[(size * (col - 1) - (col-1)*(col-2)/2) + row - col] = data;
        //index of ele M[5][4] in our array will be (5 + 4 + 3) + 1 = 13
}

int lowerTriangle::get(int row, int col)
{
    if (row >= col)
        return arr[(size * (col - 1) - (col-1)*(col-2)/2) + row - col];
    else
        return 0;
}
...
void lowerTriangle::display()
{
    for (int row = 1; row <= size; row++)
    {
        ...
        {
            if (row >= col)
                cout<<arr[(size * (col - 1) - (col-1)*(col-2)/2) + row - col]<<" ";
            else
                cout<<"0 ";
        }
